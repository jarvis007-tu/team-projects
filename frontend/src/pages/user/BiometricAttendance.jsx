    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4">
      <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-green-100">
                Mark your attendance with fingerprint
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {/* Location Status */}
          <div className={`p-4 rounded-xl ${location ? 'bg-green-50 dark:bg-green-900/20' : 'bg-yellow-50 dark:bg-yellow-900/20'}`}>
            <div className="flex items-center gap-3">
              <MdLocationOn className={`w-6 h-6 ${location ? 'text-green-600' : 'text-yellow-600'}`} />
              <div>
                <p className={`text-sm font-medium ${location ? 'text-green-700 dark:text-green-400' : 'text-yellow-700 dark:text-yellow-400'}`}>
                  {locationLoading ? 'Getting location...' : location ? 'Location Enabled' : 'Location Unavailable'}
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Enrollment Status */}
          <div className={`p-4 rounded-xl ${enrollmentStatus?.is_enrolled ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'}`}>
            <div className="flex items-center gap-3">
              <MdFingerprint className={`w-6 h-6 ${enrollmentStatus?.is_enrolled ? 'text-green-600' : 'text-red-600'}`} />
              <div>
                <p className={`text-sm font-medium ${enrollmentStatus?.is_enrolled ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'}`}>
                  {loading ? 'Checking...' : enrollmentStatus?.is_enrolled ? 'Fingerprint Enrolled' : 'Not Enrolled'}
                </p>
              </div>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6">
                You need to enroll your fingerprint before using biometric attendance.
              </p>
              <a
            ...
            <div className="space-y-6">
              {/* Result Display */}
              {attendanceResult && (
                <div className={`p-4 rounded-xl ${attendanceResult.success ? 'bg-green-50 dark:bg-green-900/20' : attendanceResult.isNoMealTime ? 'bg-orange-50 dark:bg-orange-900/20' : 'bg-red-50 dark:bg-red-900/20'}`}>
                  {attendanceResult.success ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="bg-white dark:bg-gray-700 rounded-lg p-3">
                        <p className="text-xs font-medium text-gray-600 dark:text-gray-300 mb-2">Meal Timings:</p>
                        <div className="grid grid-cols-3 gap-2 text-center">
                          <div className="p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded">
                            <p className="text-lg">🍳</p>
                            <p className="text-xs font-medium text-gray-700 dark:text-gray-300">Breakfast</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">7-10 AM</p>
                          </div>
                          <div className="p-2 bg-orange-50 dark:bg-orange-900/20 rounded">
                            <p className="text-lg">🍛</p>
                            <p className="text-xs font-medium text-gray-700 dark:text-gray-300">Lunch</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">12-3 PM</p>
                          </div>
                          <div className="p-2 bg-purple-50 dark:bg-purple-900/20 rounded">
                            <p className="text-lg">🍽️</p>
                            <p className="text-xs font-medium text-gray-700 dark:text-gray-300">Dinner</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">7-10 PM</p>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <button
                ...
                className="w-full flex flex-col items-center justify-center gap-4 p-8 bg-gradient-to-r from-green-600 to-teal-600 text-white rounded-2xl hover:from-green-700 hover:to-teal-700 transition-all disabled:opacity-50 shadow-lg"
              >
                {verifying ? (
                  <>
                    <div className="w-16 h-16 border-4 border-white border-t-transparent rounded-full animate-spin"></div>
                    <span className="text-xl font-semibold">Touch Your Fingerprint Sensor...</span>
                  </>
                ) : (
                  <>
                    <MdFingerprint className="w-20 h-20" />
                    <span className="text-xl font-semibold">Tap to Mark Attendance</span>
                    <span className="text-green-100 text-sm">Touch your fingerprint sensor when prompted</span>
                  </>
                )}
              </button>
                ...
                >
                  ...
                  <div className="text-right">
                    <span className={`text-xs px-2 py-1 rounded-full ${
                      ...
                      {record.scan_method === 'fingerprint' ? 'Biometric' : 'QR Code'}
                    </span>
